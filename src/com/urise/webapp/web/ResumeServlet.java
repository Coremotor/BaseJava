...
public class ResumeServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        switch (action) {
            ...
            case "edit":
                r = storage.get(uuid);
                for (SectionsType type : SectionsType.values()) {
        ...
        for (SectionsType type : SectionsType.values()) {
            ...
            if (HtmlUtil.isEmpty(value) && values.length < 2) {
                ...
                switch (type) {
                    ...
                    case ACHIEVEMENT:
                    case QUALIFICATIONS:
                        List<String> sections = Arrays.stream(value.replaceAll("\r", "")
                                        .split("\n"))
                                .filter(str -> !str.isEmpty()).toList();
                        r.addSection(type, new ListSection(sections));
                        break;
                    case EDUCATION:
                    case EXPERIENCE:
                            ...
                            if (!HtmlUtil.isEmpty(name)) {
                                ...
                                for (int j = 0; j < titles.length; j++) {
                                    if (!HtmlUtil.isEmpty(titles[j])) {
                                        positions.add(new Company.Period(DateUtil.parse(startDates[j]),
